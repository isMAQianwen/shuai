...
# Metric, loss .etc
from model.utils_twoflow_demo import *
...
def parse_args():
    ...
    parser.add_argument('--img_demo_dir_reverse', type=str, default='img_demo_reverse',
                        help='img_demo')
    parser.add_argument('--img_demo_index', type=str,default='001326',
                        help='target1, target2, target3')
    parser.add_argument('--img_demo_index_reverse', type=str,default='001326',
                        help='target1, target2, target3')
...
class Trainer(object):
    def __init__(self, args):
        ...
        img_dir_reverse   = args.img_demo_dir_reverse+'/'+args.img_demo_index_reverse+args.suffix
        yangben = [img_dir, img_dir_reverse]
        ...
        checkpoint      = torch.load(r'.\NUDT-DNA-weights\NUDT-DNA-twoflownew.tar',map_location=device)
        self.model.load_state_dict(checkpoint['state_dict'])

        # Test
        self.model.eval()
        img = img.cuda()
        img_reverse = img_reverse.cuda()
        img = torch.unsqueeze(img,0)
        img_reverse = torch.unsqueeze(img_reverse, dim=0)

        if args.deep_supervision == 'True':
            preds = self.model(img,img_reverse)
            pred  = preds[-3]
            pred = pred.to('cpu')
            ...
            cax = ax.imshow(pred_np_jet, cmap='jet')

            # Add a colorbar to the right of the image
            cbar = fig.colorbar(cax, fraction=0.025)
            ...
            pred  = self.model(img, img_reverse)
