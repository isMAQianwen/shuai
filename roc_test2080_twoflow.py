...
class Trainer(object):
    def __init__(self, args):
        ...
        self.PD_FA = PD_FA_pix2(1,args.ROC_thr)
        self.mIoU  = mIoU(1)
        self.nIoU_metric = SamplewiseSigmoidMetric(1, score_thresh=0.55)
        ...
        with torch.no_grad():
            ...
            FA, PD = self.PD_FA.get()
            scio.savemat(dataset_dir + '/' +  'roc_value_result'+ '/' +args.st_model  + '_PD_FA_' + str(255),
                         {'number_record1': FA, 'number_record2': PD})

            save_result_for_test(dataset_dir, args.st_model,args.epochs, mean_IOU, recall, precision)
            save_result2_for_test(dataset_dir, args.st_model, args.epochs, niou, FA, PD)
