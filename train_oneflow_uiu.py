...
from model.metric_raw import *
from model.loss import *
from model.load_param_data import  load_dataset, load_param

# model
# from model.model_DNANet_mouyiban import  Res_CBAM_block
# from model.model_DNANet_mouyiban import  DNANet
from model.uiunet import UIUNET
torch.cuda.set_device(0)   #使用显卡１进行训练，这样远程不卡
class Trainer(object):
    def __init__(self, args):
        ...
        self.test_data  = DataLoader(dataset=testset,  batch_size=args.test_batch_size, num_workers=args.workers,drop_last=False)  #改为ｔｒｕｅ
        ...
        if args.optimizer   == 'Adam':
            ...
            self.optimizer  = torch.optim.Adagrad(filter(lambda p: p.requires_grad, model.parameters(),), lr=0.001)#args.lr
            #self.optimizer = optim.Adam(model.parameters(), lr=0.001, betas=(0.9, 0.999), eps=1e-08, weight_decay=0)
        if args.scheduler   == 'CosineAnnealingLR':
            self.scheduler  = lr_scheduler.CosineAnnealingLR( self.optimizer, T_max=args.epochs, eta_min=args.min_lr)#args.min_lr
        #self.scheduler.step()
    ...
    def training(self,epoch):
        ...
        for i, ( data, labels) in enumerate(tbar):
            ...
            if args.deep_supervision == 'True':
               ...
               loss = SoftIoULoss(pred, labels)  #得到损失
            self.optimizer.zero_grad()
            loss.backward()     #损失反向传播对模型参数求梯度
            self.optimizer.step()#优化器利用梯度以及学习率更新参数
