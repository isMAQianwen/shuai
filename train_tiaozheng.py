...
from model.yizhaonet import RDN
...
class Trainer(object):
    def __init__(self, args):
        ...
        if args.optimizer   == 'Adam':
            ...
            self.optimizer1 = optim.Adam(filter(lambda p: p.requires_grad, net.parameters()), lr=args.lr)
        elif args.optimizer == 'Adagrad':
            self.optimizer  = torch.optim.Adagrad(filter(lambda p: p.requires_grad, model.parameters()), lr=args.lr)
            self.optimizer1 = torch.optim.Adagrad(filter(lambda p: p.requires_grad, net.parameters()), lr=args.lr)
        if args.scheduler   == 'CosineAnnealingLR':
            self.scheduler  = lr_scheduler.CosineAnnealingLR( self.optimizer, T_max=args.epochs, eta_min=args.min_lr)
            self.scheduler1 = lr_scheduler.CosineAnnealingLR( self.optimizer1, T_max=args.epochs, eta_min=args.min_lr)
    ...
    def training(self,epoch):
        ...
        for i, ( data, labels) in enumerate(tbar):
            ...
            if args.deep_supervision == 'True':
                ...
                preds1 = self.net(data)
                loss = 0
                for pred in preds:
                    pred += preds1
                    loss += SoftIoULoss(pred, labels)
                loss /= len(preds)
                #loss1 = SoftIoULoss(preds1, labels)
            else:
